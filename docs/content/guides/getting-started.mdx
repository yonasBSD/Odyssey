...
- uses the `session-window/v1` memory provider
- sets the default model in `agents/<bundle-id>/agent.yaml` to `openai / gpt-4.1-mini`
- lists all builtin tools explicitly in `manifest.tools`
- writes a bundle-local `.gitignore` so build artifacts such as `target/` and `node_modules/` stay out of bundle payloads
